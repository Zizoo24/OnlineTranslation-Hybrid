...
async function exploreFolder(folderName) {
  ...
  try {
    const result = await client.api(`/me/drive/root:/${folderName}:/children`).get();
    console.log(`\n${folderName} contents:`);
...
await exploreFolder('OnlineTranslationae');
await exploreFolder('Pictures/Screenshots');
